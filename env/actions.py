class ActionParser:
    ...
    def parse(self, action):
        ...
        if action_type == 'write_file':
            if '|' not in payload:
                return {'type': 'write_file', 'args': {'path': payload, 'content': None}}
            path, content = payload.split('|', 1)
            return {'type': 'write_file', 'args': {'path': path.strip(), 'content': content}}
...
class ActionExecutor:
    ...
    def execute(self, action):
        try:
            if self.state is None:
                return {'output': '', 'error': 'state unavailable'}
            action_type = action.get('type')
            args = action.get('args', {})
            if action_type == 'run_command':
            ...
            if action_type == 'write_file':
                ...
                return {'output': f'Wrote {len(content)} bytes to {path}', 'error': ''}
            if action_type == 'kill_port':
                port = args.get('port')
                if port in self.state.ports and self.state.ports.get(port) == 'occupied':
                    self.state.update_port(port, 'free')
                    return {'output': f'Freed port {port}', 'error': ''}
                return {'output': '', 'error': f'Port {port} is not in use'}
            if action_type == 'restart_service':
                service = args.get('service', '')
                if service not in self.state.services:
                    return {'output': '', 'error': f"Service '{service}' not found"}
            ...
            if action_type == 'check_status':
                ...
                return {'output': status_output, 'error': ''}
            return {'output': '', 'error': 'unsupported action type'}
    ...
    def _execute_run_command(self, command):
        ...
        if command.startswith('kill_port '):
            ...
            if not port_text.isdigit():
                return {'output': '', 'error': 'Invalid port'}
            port = int(port_text)
            if port in self.state.ports:
                self.state.update_port(port, 'free')
                return {'output': f'Freed port {port}', 'error': ''}
            return {'output': '', 'error': 'Invalid port'}
        return {'output': '', 'error': 'unsupported run_command'}
    ...
    def _read_file(self, path):
        ...
        return {'output': '', 'error': f'cat: {path}: No such file or directory'}
    ...
    def _restart_service(self, service):
        ...
        if port is None:
            ...
            return {'output': f'Job for {service}.service failed.', 'error': f'Failed to restart {service}: port is occupied'}
        ...
        return {'output': f'Restarted {service}. Service is running.', 'error': ''}
